...
$data = array();
$q="
SELECT id,date,type,centili,sum(quantity) as quant,texte,client FROM
(
SELECT 
bon_id as id,
bon_date as date,
...
if($result=$mysqli->query($q)){
    ...
    while ($row = $result->fetch_assoc()) {
	    $data[]=$row["id"].';'.$row["date"].';'.$row["type"].';'.utf8_encode($row["centili"]).';'.utf8_encode($row["quant"]).';'.utf8_encode($row["texte"]).';'.utf8_encode($row["client"]);
        $index++;
    }
    if($index==0){
        echo "-1";
